...
def godel_metric_max(r_6d, psi, psi_past, t):
    ...
    ctc_term = 1e-4 * fractal_dimension_max(phi_N, r_6d, t) * np.abs(psi_past)**2 / (np.abs(psi)**2 + 1e-10) * np.exp(1j * T_c * np.tanh(np.angle(psi) - np.angle(psi_past)))
...
def plot_zpe_density(psi, phi_N, t, psi_past):
    ...
    sigma = 0.5 * np.log2(N**3)
    g_00 = -1
    zpe = zpe_density_max(r_6d, phi, sigma, g_00, psi, psi_past, dx, t)
    fig = plt.figure()
...
for t_idx in range(n_iterations):
    ...
    zpe = zpe_density_max(r_6d, np.arctan2(coords[:,1], coords[:,0]), 0.5 * np.log2(N**3), -1, psi, psi_past, dx, t)
